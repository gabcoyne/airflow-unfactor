...
def generate_flow_tests(
    ...
    for original_id, func_name in task_mapping.items():
        lines.extend([
            ...
            '        # Call the task function directly (not as a Prefect task)',
            f'        result = {func_name}.fn()',
            '        ',
            '        # TODO: Add specific assertions based on expected behavior',
            '        # This is a smoke test to verify the function runs',
            '        assert result is not None or result is None  # Adjust based on expected return',
            '',
            f'    def test_{func_name}_as_task(self, prefect_harness):',
            f'        """Verify {func_name} works as a Prefect task."""',
            '        # Run as a Prefect task to verify decorator behavior',
            f'        result = {func_name}()',
            '        ',
            '        # Prefect tasks return PrefectFuture in some contexts',
            '        # but in sync context they return the value directly',
            '        assert True  # Task executed without error',
            '',
        ])
    ...
    lines.extend([
        ...
        '        # TODO: Add assertions about task execution order',
        '        # This may require inspecting flow run results',
        f'        result = {flow_name}()',
        '        assert True',
        '',
    ])
    ...
    lines.extend([
        ...
        '        # Count tasks in the flow',
        '        # Note: In Prefect, tasks are functions, not objects in a DAG',

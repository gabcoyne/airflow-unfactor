...
The full operator coverage matrix is generated from the operator registry. Run:

```bash
python -c "from airflow_unfactor.converters.registry import generate_operator_docs; print(generate_operator_docs())"
```

This ensures documentation stays in sync with actual converter capabilities.
